...
import {
  ...
  postTransactionsBatch as postBankTransactionsBatch,
...
const FORM_FIELDS: { key: NumericFormField; label: string }[] = [
  ...
  { key: "cc_processing_fees", label: "Credit Card Processing Fees" },
...
const DEFAULT_QB_MAPPINGS: QBMappingRow[] = [
  ...
  { qb_account_name: "Software & Subscriptions", laundrocfo_field: "software_subscriptions" },
  { qb_account_name: "Credit Card Processing Fees", laundrocfo_field: "cc_processing_fees" },
...
export default function FinancialsPage() {
  ...
  async function saveMonthlyRecord() {
    ...
    try {
      const payload = {
        ...
        software_subscriptions: toNum(form.software_subscriptions),
        cc_processing_fees: toNum(form.cc_processing_fees),
  ...
  async function confirmPostAppliedRuleTransactions() {
    ...
    try {
      const result = await postBankTransactionsBatch(supabase, {
        storeId: store.id,
        userId,
        transactions: rulePostPrompt.transactions,
        existingRecords: records as MonthlyFinancialRecord[],
  ...
  async function saveCategorizationRule() {
    ...
    if (ruleFormType === "vendor") {
      ...
      try {
        ...
        setRuleFormKey(null);
        await promptApplyRuleToExisting(rule, ruleFormCategory);
      } catch (err) {
