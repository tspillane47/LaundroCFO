...
type ColorZone = { start: number; end: number; color: string; darkGlow: string };
...
function DashboardDial({
  ...
  return (
    <div
      className={clsx(
        "rounded-2xl border p-5 flex flex-col items-center transition-colors",
        "bg-white border-slate-200",
        "dark:bg-[#111827] dark:border-slate-700/60",
        !hasData && "opacity-60"
      )}
    >
      <div className="text-[13px] font-semibold tracking-wide uppercase text-slate-500 dark:text-slate-400 mb-1 text-center">
        {label}
      </div>
      ...
      <div className="relative w-full flex justify-center" style={{ height: size * 0.62 }}>
        ...
        >
          {hasData ? (
            <>
              <div className="text-[32px] font-bold leading-none text-slate-900 dark:text-white tabular-nums">
                {displayValue}
              </div>
              {displayGrade && (
                <div
                  className={clsx(
                    "mt-2 w-9 h-9 rounded-full flex items-center justify-center text-[15px] font-bold",
